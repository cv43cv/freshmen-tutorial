...
def eval_voc_detection(savename, dataset, ovthresh=0.5):
    boxes = {}
    for classname in range(1, 21):
        ...
        for line in lines:
            ...
            if data[0] in boxes:
                boxes[data[0]].append(d+[float(classname)])
            else:
                boxes[data[0]] = [d+[float(classname)]]
    pred_bboxes = []
    pred_labels = []
    pred_scores = []
    gt_bboxes = []
...
def calc_detection_voc_prec_rec(
    ...
    for pred_bbox, pred_label, pred_score, gt_bbox, gt_label, gt_difficult in \
        ...
        for l in np.unique(np.concatenate((pred_label, gt_label)).astype(int)):
            ...
            iou = jaccard_np(pred_bbox_l, gt_bbox_l)
